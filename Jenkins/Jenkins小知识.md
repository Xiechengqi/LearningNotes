...
1、修改 `/etc/sysconfig/jenkins` 文件

``` shell
JENKINS_PORT="9090"
```

2、Jenkins Web 界面修改 - **`系统管理` -> `系统设置` -> `管理监控配置` -> `JenKins Location`**

![](./images/jenkins_modify_port.jpg)
...
![](http://xcq.cn:38084/buildStatus/icon?job=Pipeline-test&config=lastSuccessVersion)  ![](http://xcq.cn:38084/buildStatus/icon?job=Pipeline-test&config=pendingVersion) 
